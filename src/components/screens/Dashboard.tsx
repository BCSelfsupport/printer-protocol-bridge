...
export function Dashboard({
  ...
  const { canDatabase, isDemo } = useLicense();
        ...
        <div className="flex-1 overflow-x-auto mx-2 md:mx-4">
          <div className="flex gap-1 justify-center min-w-max px-2">
            ...
            {onHome && (
              <button
                onClick={onHome}
                className="flex flex-col items-center justify-center gap-1 min-w-[72px] md:min-w-[80px] px-3 md:px-5 py-2 md:py-2.5 rounded transition-all industrial-button text-white"
              >
            ...
            {[
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1 min-w-[72px] md:min-w-[80px] px-3 md:px-5 py-2 md:py-2.5 rounded transition-all ${
                  item.disabled
...
function MessagePreviewCanvas({ message, printerTime, messageContent }: MessagePreviewCanvasProps) {
  ...
  useEffect(() => {
    ...
    if (messageContent && messageContent.fields.length > 0) {
      ...
      // Fields fetched via ^LF are already stored in absolute 32-row canvas
      // coordinates (Y inversion applied by buildMessageDetails). No additional
      // vertical offset is needed — render at stored positions directly.
