    ...
    sections: [
      {
        ...
        screenshot: '/manual-screenshots/50-twin-overview.png',
        callouts: [
          { label: 'Top toolbar', text: 'Training, HUD/Debug toggle, Bind two printers, Start generator (synthetic), Clear, CSV/JSON export, Replay' },
          { label: 'Production run bar', text: 'shows Catalog / Twin pair / LIVE checklist with Pre-flight and Start production run actions' },
          { label: 'Catalog tiles', text: 'Catalog total / Remaining / Printed / Miss-prints — live ledger counters that survive a reload' },
          { label: 'Selected messages preview', text: 'visual cross-check — DataMatrix 16×16 on the LID side, 7×5 dot-matrix text on the SIDE side; must match each printer\'s HMI before going LIVE' },
          { label: 'M / FT toggle + Alarm', text: 'metric/imperial conveyor units and the audible miss alarm on/off' },
        ],
      },
      {
        id: 'twin-binding',
        ...
        screenshot: '/manual-screenshots/51-twin-bind-dialog.png',
        callouts: [
          { label: 'Printer A (Lid)', text: 'IP/port + Test Connection — this printer prints the ECC200 Data Matrix on the bottle lid via ^BD' },
          { label: 'Printer B (Side)', text: 'IP/port + Test Connection — this printer prints the human-readable text on the side via ^TD' },
      ...
      },
      {
        ...
        body: `Before going LIVE, click **Pre-flight** to dispatch ghost cycles through the bonded path with the placeholder serial \`DRYRUN0000000\`. No catalog serials are consumed and nothing is written to the production-run ledger.\n\nThe dialog reports per-cycle wire RTT, A/B skew, and full cycle time, plus a pass/fail verdict. A passing pre-flight means: both printers acked the ^MD subcommand, both reached the \`C\` (committed) state, and the bonded cycle completed under the 300 ms target. Re-run pre-flight any time you change a message, swap a printer, or move to a new line.`,
        screenshot: '/manual-screenshots/52-twin-preflight.png',
        callouts: [
          { label: 'Mode banner', text: 'shows whether tests run against the real bonded pair or the SYNTH timing model when no pair is bound' },
          { label: 'Cycles slider', text: 'how many ghost dispatches to fire (default 8) — more cycles = tighter timing distribution but longer wait' },
          { label: 'Run pre-flight', text: 'fires the dispatches; results appear inline with per-cycle RTT, A/B skew, and pass/fail verdict' },
          { label: 'Cancel', text: 'closes the dialog without firing — safe at any time, no state is written' },
        ],
      },
      {
        id: 'twin-live-run',
        title: 'Going LIVE — the operator HUD',
        body: `Once a pair is bound and a catalog is loaded, click **Start generator** (or **Go LIVE** when bonded) to open the Operator HUD. The HUD is designed for floor visibility from 2-3 meters:\n\n- **Big BPM** (bottles per minute) — current and 60-second rolling average alongside the conveyor speed in m/min\n- **Last serial** in monospace, with A/B status lights (green = both committed, amber = one acked, red = miss)\n- **Audible miss alarm** — sounds when a serial fails to commit on either side; toggle from the Alarm ON / OFF button\n- **Production strip** at the bottom shows live BPM, speed, pitch, bottle diameter, and gap — the full conveyor profile at a glance\n- **A · LID / B · SIDE / SYNTH tiles** — per-side bind status; SYNTH replaces both when running synthetic profiles\n\nThe HUD doubles as a screensaver — switch to **Screen** mode for fullscreen line-side display.`,
        screenshot: '/manual-screenshots/53-twin-hud-throughput.png',
        callouts: [
          { label: 'Throughput gauge', text: 'big BPM number + rolling 60-second m/min average — visible from across the production floor' },
          { label: 'Last printed panel', text: 'most recent serial with cycle ms and A/B skew in the corner — quick health check at a glance' },
          { label: 'A · LID / B · SIDE tiles', text: 'per-side bind state; turn green when committed, amber on partial ACK, red on miss' },
          { label: 'SYNTH tile', text: 'shown when running the synthetic timing profile (no real pair bound) for training/diagnostics' },
          { label: 'Production strip', text: 'live BPM, speed (m/min), pitch, bottle diameter ⌀, and gap — the full conveyor profile at the bottom of the screen' },
        ],
      },
      {
        id: 'twin-production-run',
        title: 'Production runs and lot locking',
        body: `A production run binds catalog consumption to a specific **Lot Number** and **Operator**. From the Production Run bar:\n\n1. Click **Start production run** — enter Lot, Operator, and an optional note\n2. The run is locked to the catalog fingerprint at start; a new catalog import during a run is rejected\n3. Every committed serial is appended to the cloud ledger with the run ID\n4. Click **End Run** to close it out — CodeSync offers signed CSV / JSON export with a SHA-256 of the manifest for compliance audit\n\nIf the application crashes or the PC reboots mid-run, the **Ledger Resume Banner** offers to resume from the last committed bottle.`,
      },
      {
      ...
      },
      {
        id: 'twin-training',
        title: 'Training mode',
        body: `New operators can practice on a simulated conveyor without touching real printers or consuming real catalog serials. Open **Training** from the Twin Code top bar — the menu offers a full ~11 minute end-to-end tour or quick replay of any single stage.\n\nThe tour covers four stages:\n\n1. **Bind & auto-create** (~3 min) — pair two printers as one logical unit (LID + SIDE)\n2. **Message preview cross-check** (~2 min) — confirm A and B are loaded with the right messages\n3. **Pre-flight dry run** (~2 min) — 5 real bonded dispatches before going LIVE\n4. **Live run + fault recovery** (~4 min) — go LIVE, watch the HUD, recover from faults\n\nThe simulator drives the same dispatcher and HUD code as production, so muscle memory transfers 1:1.`,
        screenshot: '/manual-screenshots/54-twin-training.png',
        callouts: [
          { label: 'Run full end-to-end tour', text: '~11 minute guided walkthrough covering bind → preview → preflight → live run → fault recovery' },
          { label: 'Replay a stage', text: 'jump straight to any single stage — perfect for refresher training without sitting through the whole tour' },
          { label: 'Stage durations', text: 'each stage is timed (3m / 2m / 2m / 4m) so trainers can plan around shift changes' },
          { label: 'Stage description', text: 'one-line summary of what the stage teaches — helps operators self-select what they need to brush up on' },
        ],
      },
    ],
  },
