...
const EMULATED_PRINTERS: EmulatedPrinterConfig[] = Array.from({ length: 13 }, (_, i) => {
  const id = i + 1;
  return {
    id,
    name: `Printer ${id}`,
    ipAddress: `192.168.1.${54 + id}`,
    port: 23,
    model: id === 1 ? 'N-88 STD' : 'N-86 STD',
    initialState: {
      currentMessage: 'BESTCODE',
      printCount: 1000 + id * 137,
      productCount: 5000 + id * 421,
      inkLevel: 'FULL',
      makeupLevel: 'FULL',
      // All emulated printers start online + ready (jet running + HV on)
      // so the fleet view shows a full 13-printer online network by default.
      hvOn: true,
      jetRunning: true,
    },
  };
